...
public class UI {
    ...
    public Benutzer leseUserInfo()
    {
        ...
        while(!readSport.equals(quit))
        {
            if (readSport.equals("f"))
                list.add(fussball);
            if (readSport.equals("b"))
                list.add(basketball);
            if (readSport.equals("h"))
                list.add(hochsprung);
            if (readSport.equals("H"))
                list.add(hindernislauf);
            if (!readSport.equals("H") && !readSport.equals("h") && !readSport.equals("f") && !readSport.equals("b"))
                System.out.println("Bitte geben Sie gultige Daten ein");

            readSport=in.nextLine();
        }
